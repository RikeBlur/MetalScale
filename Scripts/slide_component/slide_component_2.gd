class_name slide_component_2
extends Node2D

@export var area_array: Array[Area2D]
@export var config: int = 2
...
func _setup_connections() -> void:
	if config == 2 and area_array.size() == 5:
		var a1 = area_array[0]
		var a2 = area_array[1]
		var a3 = area_array[2]
		var a4 = area_array[3]
		...
		if a5 and a5.has_signal("clicked_signal"):
			a5.clicked_signal.connect(_on_a5_chosen)
