# Dialogue System
不位于 Canvas Layer 渲染层，z-index=9/10

## 基本组件 Dialogue（Style）
res://Scripts/system/interact/dialogue_dual.gd
res://Scripts/system/interact/dialogue_base.gd
### 重要参数
...
### 核心功能
Process ： 当 next_item==1 时，处理新的 DialogueResource。<br>

_funtion_resource ： 从 目标节点  传入 目标参数 调用 目标函数 ；如果 wait_for_signal_to_continue！= null ，
将其作为 signal_name，如果 目标节点 存在对应的signal，在该 signal 触发之前保持等待。<br>

_choice_resource ： 将 resource 中的 text 传入 dialogue_label， botton_container 显示并将 choice_text 作为label
...
_text_resource ： 将 resource 中的 text 传入 dialogue_label， 播放 sprite_animation_name 对应的立绘动画，
从0开始匀速显示文本字符，如果按下 skip ，直接显示全部字符；然后进入等待，直到再次按下 skip。

反转 dark/light ： 双人对话时，正在说话的人的立绘和对话框在上面，z-index=10，设置立绘和对话框的 material（对比度亮度）
...
### 主要方法
_trigger_source_connect : 将每个 TriggerSource 的 InteractedComponent 的信号和 DialogueManager 连接。
实现 在 TriggerSource 范围内时显示 DialogueReminder ； 交互后消除 DialogueReminder 并触发 Dialogue。
值得注意，在 _on_triggered 方法中需要确认单次触发的 TriggerSource 在触发后和 InteractedComponent 相关信号解绑。<br>

_spawn(_dual)_dialogue ： 在 process 过程中，遍历到 flag==1 的 TriggerFlag 时，通过提供的 模板（style）；
起点终点（从 DialogueContent 提取 DialogueResource）；A/B对话检索（双人） 生成对于的 Dialogue 实例。
...
text: 对话文本
text_speed： 对话显示速度
text_sound ： 单字音效（音色）；text_volume_db（音量）；text_volume_pitch_min/max（随机音调）
