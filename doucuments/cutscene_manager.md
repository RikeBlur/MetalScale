...
`CutsceneManager` 负责过场动画注册、播放、淡入淡出、跳过、死亡过场，以及过场内部的轻量信号回调系统。
...
- 播放时禁用玩家移动和交互，结束后恢复。
- 把 cutscene 挂到当前场景的 `CanvasLayer` 上。
- 控制淡入淡出。
- 长按 `ESC` 跳过当前 cutscene。
...
`GameManager.start_new_game()` 中会调用：
...
因此当前 `"test"` 就是新游戏开场 cutscene。要换开场动画，通常改 `cutscene_scenes["test"]` 或改 `GameManager.start_new_game()` 中传入的 key。
...
1. 创建一个新的 `.tscn`。
2. 根节点建议是 `Control`，挂 `Scripts/system/view/cutscene/cutscene.gd`，也就是 `Cutscene`。
3. 如果需要自定义淡入时间，设置根节点的 `fadein_time`。
4. 子节点播放完自己那部分后，发出 `cutscene_finished_partly`。`Cutscene` 会等所有带这个信号的子 Control 都完成后，发出总的 `cutscene_finished`。
...
做法二：新增 key，然后修改 `GameManager.start_new_game()`：
...
`CutsceneManager` 提供 key 到 Callable 列表的系统：

```gdscript
CutsceneManager.register_cutscene_signal("open_door")
CutsceneManager.connect_cutscene_signal("open_door", Callable(self, "_on_open_door"))
CutsceneManager.emit_cutscene_signal("open_door")
```

这不是 Godot 原生 signal 声明，而是内部字典 `_cutscene_signal_map`。适合 cutscene 内部某个时间点触发外部逻辑。
